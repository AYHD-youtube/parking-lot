...
def process(command_params):
    ...
    if command == 'Create_parking_lot':
        ...
        assert len(command_with_params) == 4, "park needs registration number and age as well"
        assert command_with_params[3].isdigit() is True, "age should be 'integer type'"
        ...
        assert len(command_with_params) == 2, "vehicle_registration_number_for_driver_of_age needs age as well"
        
        parking_lot.registration_numbers_for_drivers_with_age(command_with_params[1])
    elif command == 'Slot_numbers_for_driver_of_age':
        assert len(command_with_params) == 2, "slot_numbers_for_driver_of_age needs age as well"
        
        parking_lot.slot_numbers_for_driver_of_age(command_with_params[1])
    elif command == 'Slot_number_for_car_with_number':
        assert len(command_with_params) == 2, "Slot_number_for_car_with_number needs registration_number as well"
